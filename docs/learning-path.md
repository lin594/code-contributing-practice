...
这门课不是命令词典，而是一条包含九关、逐渐增加协作复杂度的路线。每关只把一个主要能力从“见过”变成“做过”，后续关卡会继续使用前面学过的动作。
...
| [9 CI 诊断](lessons/09-ci-diagnostics.md) | 维护同一 PR | 读取并验证自动检查 | 失败 SHA、新 head、绿色检查、验证评论 |
...
4. **读自动证据**：确认检查对应当前 head；先处理“必须修复”，红灯需要时进入 Details 看日志，再理解 Warning。
...
完成第 9 关后，找一个课程小组仓库或带 `good first issue`、`documentation`、`help wanted` 标签的开源仓库。先读对方的 README、CONTRIBUTING 和测试说明，再使用它规定的分支、commit、PR、CI 和 Review 流程。本课程的规则是练习脚手架，不会覆盖目标项目自己的约定。
