...
本关不再引入新命令，而是把清晰 PR、上游同步、Review 修改、评论回复和作者自测组合成一次模拟团队协作。第一次检查不会通过；你需要根据不断变化的共同上下文推进原 PR。
...
通过后，你已经具备维护协作上下文的核心动作。继续进入[第 9 课：从 CI 失败中定位问题](09-ci-diagnostics.md)，学习怎样独立读取自动检查证据，再完成整条课程路线。
